...
def best_focus_image(image_or_path, patch_size=None, return_heightmap=False, test = None):
    ...
    # 5. Continuous Surface Sampling
    # matth: Replaced Patch Blending with Continuous Surface Sampling.
    # Treat the focal surface as a continuous manifold Z(y,x) and sample the volume.
    # This eliminates patch boundary artifacts and provides a mathematically consistent reconstruction.
    ...
    # Create interpolator for the depth map
    ...
    # Interpolate depth map to full resolution
    try:
    ...
    # Sample the 3D volume at (Z, Y, X)
    # Coordinates array: (3, H, W)
    # We construct the coordinate volume for map_coordinates
    # Order: (Z-coord, Y-coord, X-coord)
    coords = np.stack([height_map_full, GY, GX], axis=0)

    # Use float32 for processing to save memory (if input is float64) or precision (if input is int)
    # map_coordinates uses spline interpolation (order=3)
    # mode='reflect' is standard for spatial, and works for Z if we assume continuity at boundaries.
    # However, 'nearest' is safer for Z to avoid "wrapping" or "reflecting" focus beyond the stack.
    # Since we can only pick one mode, and Z is clamped to [0, Z-1] by scoring, 'reflect' is fine
    # (it won't trigger unless we exceed bounds, which we shouldn't).

    final_img = map_coordinates(img.astype(np.float32), coords, order=3, mode='reflect')

    # Clamp to avoid ringing (negative values) from cubic spline undershoot
