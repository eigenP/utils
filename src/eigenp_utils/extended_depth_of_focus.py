...
# ]
# ///
### extended depth of focus with continuous surface sampling or patch blending
...
def best_focus_image(
    image_or_path,
    patch_size=None,
    return_heightmap=False,
    test=None,
    method: Literal['patches', 'continuous'] = 'patches'
):
    '''
    Reconstruct an extended depth of focus (EDoF) image from a Z-stack.

    Parameters:
    - image_or_path: (Z, Y, X) numpy array or path to image.
    - patch_size: Size of patches for focus estimation. Defaults to min_dim // 10.
    - return_heightmap: If True, returns (edof_image, height_map).
    - method: 'patches' (legacy blending) or 'continuous' (surface sampling).
              'patches' is default for backward compatibility.
    '''
    # 1. Load the image
