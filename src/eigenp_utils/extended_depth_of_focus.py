...
def _sample_volume_cubic(volume, z_map):
    """
    Vectorized cubic interpolation of a 3D volume along the Z-axis.
    volume: (Z, Y, X)
    z_map: (Y, X) float indices
    Returns: (Y, X) interpolated intensity
    """
    Z_dim, H, W = volume.shape

    # Ensure z_map matches volume dimensions
    if z_map.shape != (H, W):
         raise ValueError(f"z_map shape {z_map.shape} does not match volume spatial dims {(H, W)}")

    z_floor = np.floor(z_map).astype(np.int32)
    ...
    # We need p0, p1, p2, p3
    # Cast to float32 to prevent overflow during cubic interpolation (especially for uint8/uint16 inputs)

    # p1 at z_floor
    z1 = np.clip(z_floor, 0, Z_dim - 1)
    p1 = volume[z1, grid_y, grid_x].astype(np.float32)

    # p2 at z_floor + 1
    z2 = np.clip(z_floor + 1, 0, Z_dim - 1)
    p2 = volume[z2, grid_y, grid_x].astype(np.float32)

    # p0 at z_floor - 1
    z0 = np.clip(z_floor - 1, 0, Z_dim - 1)
    p0 = volume[z0, grid_y, grid_x].astype(np.float32)

    # p3 at z_floor + 2
    z3 = np.clip(z_floor + 2, 0, Z_dim - 1)
    p3 = volume[z3, grid_y, grid_x].astype(np.float32)
...
def best_focus_image(image_or_path, patch_size=None, return_heightmap=False, test = None):
    ...
    # 5. Reconstruct Surface (Continuous Sampling)
    # matth: Use RegularGridInterpolator for spatially accurate upscaling of the depth map,
    # then sample the volume using vectorized cubic interpolation.
    # This avoids the "blocky" artifacts and geometric distortions caused by patch-based blending
    # (which assumes constant depth per patch).

    # n_patches_y/x matches height_map_small dimensions
    ...
    # We reconstruct directly to the original shape, skipping the padded intermediate.
    ...
    try:
        ...
        # Fallback for older SciPy (should accept list of points or grid)
        # Note: RegularGridInterpolator call signature varies by version
