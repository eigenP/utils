...
def plot_marker_genes_dict_on_embedding(
    ...
    **kwargs
    ...
    """
    Plots marker genes from a dictionary on the embedding.

    Args:
        adata: Annotated data matrix.
        marker_dict: Dictionary mapping group names to lists of gene names.
        use_rep: Representation to use (e.g., 'X_umap').
    ...
    for group, genes in marker_dict.items():
        ...
        if not valid_genes:
            print(f"No valid genes found for group {group}")
            continue

        print(f"Plotting markers for {group}: {valid_genes}")
        # sc.pl.embedding can return a single Axes or a list of Axes
        axs = sc.pl.embedding(
            adata,
            basis=basis,
            color=valid_genes,
