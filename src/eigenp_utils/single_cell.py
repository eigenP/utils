...
def import_obs_to_adata_from_csv(
    ...
    if overwrite_existing:
        ...
        if overlapping_cols:
            print(
                f"Columns {overlapping_cols} exist. Appending '{suffix}' suffix to new data."
            )
...
def export_cell_type_annotations(
    ...
    if color_key in adata.uns:
        ...
        if len(categories) == len(colors):
            ...
            print(
                f"Warning: Number of colors in '{color_key}' does not match categories in '{annotation_key}'. Colors not exported."
            )
    else:
