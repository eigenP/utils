...
## 2024-05-24 - [Exact LLE Weights via NNLS]
**Learning:** In Locally Linear Embedding (LLE), the optimal non-negative reconstruction weights are the solution to a Non-Negative Least Squares (NNLS) problem. The naive heuristic of solving the unconstrained system $G w = 1$ and clipping negative values to zero ($\max(w, 0)$) is mathematically unjustified and can yield suboptimal reconstructions. By reformulating the problem as $\min_w \frac{1}{2} w^T G w - 1^T w$ subject to $w \ge 0$, we can solve it exactly via `scipy.optimize.nnls` using the Cholesky decomposition of $G = C C^T$. This projects the solution onto the simplex correctly.
**Action:** When computing graph weights or nearest neighbor reconstructions that require non-negativity (and a sum-to-one constraint), use a rigorous constrained optimizer like NNLS instead of ad-hoc clipping, except when the unconstrained solution natively satisfies the constraints.
