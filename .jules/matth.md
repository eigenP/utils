...
## 2025-03-05 - [Graph Signal Processing: Self-Loops and Lazy Random Walks]
**Learning:** In `smooth_expression_on_graph` within `eigenp_utils/single_cell.py`, row-standardizing a k-nearest neighbor (kNN) graph ($W$) with zeros on the diagonal replaces a node's expression entirely with the mean of its neighbors. This effectively discards the node's own measured signal. In spectral graph theory and graph signal processing, applying such an operator can lead to bipartite oscillations rather than a stable low-pass filter. A proper low-pass filter or lazy random walk requires explicitly adding self-loops ($A + I$) prior to row-standardization, ensuring the node retains a fractional contribution of its original signal during diffusion.
**Action:** Always verify the diagonal of an adjacency or connectivity matrix before applying degree normalization for signal diffusion or smoothing. Use `W.setdiag(1.0)` followed by `W.eliminate_zeros()` to mathematically guarantee stability and signal retention.
