...
## 2024-05-27 - Exact Participation Ratio via Matrix Invariants
**Learning:** In computing the Participation Ratio ($PR = (\sum \lambda_i)^2 / \sum \lambda_i^2$) as a proxy for local manifold curvature, it is a common mathematical oversight to compute the full eigendecomposition of the local covariance matrix $C$ just to sum its eigenvalues. This imposes an unnecessary $O(d^3)$ computational cost per point. By definition, the sum of the eigenvalues of any square matrix is exactly its trace ($\text{tr}(C)$). Furthermore, for any symmetric matrix (like a covariance matrix), the sum of its squared eigenvalues is exactly its squared Frobenius norm ($\|C\|_F^2 = \sum_{i,j} C_{ij}^2$).
**Action:** Replaced the explicit $O(d^3)$ numerical eigendecomposition (`np.linalg.eigvalsh`) in `_compute_kknn_curvatures` with exact, closed-form $O(d^2)$ matrix invariants. The Participation Ratio is now computed directly and robustly as $PR = (\text{tr}(C))^2 / \|C\|_F^2$. This eliminates potential convergence issues in the eigensolver and drastically improves the performance of the adaptive $k$-NN backbone without altering the mathematical objective.
