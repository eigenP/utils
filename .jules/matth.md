...
## $(date +%Y-%m-%d) - Bias-Corrected and Accelerated (BCa) Bootstrap
**Learning:** Standard Percentile and Bias-Corrected (BC) bootstrap methods assume that the standard error of the estimator is constant across different true values of the parameter (constant variance). This is often violated in practice (e.g., when the variance is related to the mean, like with right-skewed data or correlation coefficients). The BCa method introduces an "acceleration factor" ($a$) calculated via jackknife resampling that corrects for the skewness in the distribution of the estimator and allows the standard error to vary with the parameter, making it a second-order accurate method ($O(n^{-1})$ error vs. $O(n^{-1/2})$).
**Action:** When calculating non-parametric bootstrap confidence intervals, default to BCa (or at least BC) rather than standard percentiles to ensure robustness against median bias and skewness, especially for small sample sizes or non-normal distributions.
