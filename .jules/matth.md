...
## 2024-03-26 - Exact 1D Wasserstein Distance via Order Statistics

**Learning:** When computing the 1-Wasserstein distance between two empirical 1D distributions of identical sample size $N$ with uniform weights ($1/N$), the problem admits a closed-form exact solution that perfectly equates to the $L_1$ distance between their sorted vectors (order statistics): $W_1(X, Y) = \frac{1}{N} \sum_{i=1}^N |x_{(i)} - y_{(i)}|$.
