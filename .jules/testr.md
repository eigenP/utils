...
## 2025-02-24 - Dimensionality Parser Resizing Flaw
**Learning:** The `@dimensionality_parser` decorator relied on a flawed heuristic to distinguish between "reduced" dimensions (e.g., max projection) and "resized" dimensions (e.g., downsampling). It compared input/output sizes: if sizes mismatched, the dimension was assumed reduced/deleted. This caused resizing operations (Input `[N, N]` -> Output `[N/2, N/2]`) to crash, as the parser treated both dimensions as reduced, expecting a scalar output.
**Action:** The heuristic was improved: if the *number of dimensions* (rank) is preserved (`len(input) == len(output)`), we force the parser to treat all target dimensions as preserved (resized), ignoring the size mismatch. A robustness test (`tests/test_dimensionality_parser_resizing.py`) was added to verify this resizing invariant.
