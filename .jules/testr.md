...
## 2025-02-17 - Max-Projection Orthogonality Violation in Drift Correction
**Learning:** The drift correction algorithm previously applied 2D windowing *before* computing 1D max projections. This caused a subtle violation of orthogonality: movement of an object along the Y-axis would modulate its intensity in the X-projection (via the Y-window), effectively causing it to "fade out" at the edges. For objects entering the frame, this could prevent tracking until they were deep inside the tapered region.
**Action:** Refactored `estimate_drift_2D` to compute max projections on the raw frame first, then apply 1D windowing to the resulting profiles. This ensures that the X-projection is invariant to Y-position (orthogonality) and improves tracking robustness for objects near boundaries. Tests should verify projection energy stability under orthogonal motion.
