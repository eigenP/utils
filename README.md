...
* **Publication-Ready Plots**: `raincloud_plot` supporting Seaborn-style arguments (grouped and colored with automatic position dodging), pre-KDE outlier filtering, and data subset highlighting. Custom Matplotlib colormap generation via `colormap_maker`. SVG export tools (`savefig_svg`, `set_plotting_style`) include support for editable text layers and threshold-based scatter point rasterization to minimize file sizes while preserving vector shapes.

### Single-Cell Analysis
* **Data Import**: Intelligent metadata integration via `import_obs_to_adata_from_csv`, featuring graceful error handling and column suggestions for missing metadata keys.
