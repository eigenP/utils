...
* **Intensity Rescaling**: Tools for contrast enhancement (including CLAHE), BaSiCPy-style shading correction (`fit_basic_shading`), exact analytical OLS Z-decay correction, and slice-by-slice brightness adjustments.

### Plotting & Visualization
* **Interactive 3D Widgets**: Jupyter and Marimo-compatible, `anywidget`-based orthogonal slicers (`TNIASliceWidget`, `show_xyz` for dynamic multichannel viewers) with view rotation support, interactive point cloud visualization (`IsoScatterWidget`), 3D point annotation (`TNIAAnnotatorWidget`), and 3D plotly scatter wrappers (`plotly_scatter_3d`).
* **Publication-Ready Plots**: `raincloud_plot` supporting Seaborn-style arguments (grouped and colored with automatic position dodging) and intelligent file-size optimizations via `raster_threshold`. Custom Matplotlib colormap generation via `colormap_maker`, and SVGs embedded with metadata via `savefig_svg` (also with rasterization thresholding).
...
* **Lineage Coupling**: Calculate exact lineage coupling expectations and z-scores analytically using a vectorized log-gamma formulation of the hypergeometric distribution (`calculate_lineage_coupling`).
...
* **General Statistics**: `stats.py` provides comprehensive statistical functions including `cohens_d`, `bootstrap_ci`, `summary_stats`, `remove_outliers` (supporting Mahalanobis distance outlier detection), and `add_stat_annotations` for annotating plots with significance markers.


### Core Utilities
* **Project Scheduling**: Gantt-style timeline schedules compatible with Marimo notebooks (`task_calendar_scheduler.py`).
