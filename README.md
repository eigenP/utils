...
* **Interactive 3D Widgets**: Jupyter and Marimo-compatible, `anywidget`-based orthogonal slicers (`TNIASliceWidget`, `show_zyx` for dynamic multichannel viewers) with rotatable crosshairs, interactive point cloud visualization (`show_iso_scatter`), and 3D point annotation (`TNIAAnnotatorWidget`). Features a "Copy Params" button to export UI settings as Python kwargs, robust affine-rotated crosshairs, optional `channel_labels` customization with auto-darkened text styling, and independent anisotropic pixel scaling support (`px`, `py`, `pz`).
...
* **Publication-Ready Plots**: `raincloud_plot` supporting Seaborn-style arguments (grouped and colored with automatic position dodging), pre-KDE outlier filtering, and data subset highlighting. Custom Matplotlib colormap generation via `colormap_maker`. SVGs can be exported with threshold-based scatter point rasterization to minimize file sizes and an `editable_text` parameter to ensure fonts remain modifiable in vector graphics software via `savefig_svg` and `set_plotting_style`.

### Single-Cell Analysis
* **Data Ingestion**: Smooth `import_obs_to_adata_from_csv` workflow with rich error debugging output for missing keys.
