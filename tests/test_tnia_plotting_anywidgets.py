...
def test_show_xyz_max_scatter_interactive_vmax_default():
    ...
    # single channel cont
    try:
        widget_single = show_xyz_max_scatter_interactive((Z, Y, X), channels=cont_data)
    except TypeError:
        # Fallback if signature hasn't changed locally but changed remotely
        widget_single = show_xyz_max_scatter_interactive(X, Y, Z, channels=cont_data)
    assert np.isclose(widget_single.vmax, p995)

    # provided vmax
    try:
        widget_single_set = show_xyz_max_scatter_interactive((Z, Y, X), channels=cont_data, vmax=50)
    except TypeError:
    ...
    try:
        widget_multi = show_xyz_max_scatter_interactive((Z, Y, X), channels=[cont_data, cont_data2])
    except TypeError:
    ...
    # mixed vmax
    try:
        widget_multi_mixed = show_xyz_max_scatter_interactive((Z, Y, X), channels=[cont_data, cont_data2], vmax=[80, None])
    except TypeError:
